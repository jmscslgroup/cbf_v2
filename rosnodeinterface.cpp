...
// Generated 04-May-2022 20:26:01
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"cbf_v2\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(cbf_v2_M, (NULL));
        cbf_v2_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(cbf_v2_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(cbf_v2_M)
                == (NULL)) || rtmGetStopRequested(cbf_v2_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(cbf_v2_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
